...
export function Header({ devices, isConnected }: HeaderProps) {
  ...
  return (
    <header className="flex items-center justify-end gap-6 px-8 py-8 shrink-0">
      
      {/* Device Counter */}
      <div className="flex items-center gap-3 px-4 py-2 rounded-full border border-[var(--border-glass)] bg-[var(--bg-glass)]">
        <span className="text-[0.75rem] font-bold text-[var(--text-muted)] uppercase tracking-wider">
          Active
        </span>
        <div className="flex items-baseline gap-1">
          <span className="text-[0.95rem] font-bold text-white">{onCount}</span>
          <span className="text-[0.8rem] text-[var(--text-dim)]">/ {total}</span>
        </div>
      </div>
